...
const Swirls = () => (
  <>
    <svg
      className="absolute left-0 top-0 -translate-x-1/3 -translate-y-1/3 text-pink-100 dark:text-pink-900/20"
      width="600"
      height="600"
      viewBox="0 0 600 600"
      fill="none"
      ...
      <div className="absolute inset-0 z-10">
        {images.map((image, index) => (
          <img
            ...
            src={image.src}
            alt={image.alt}
            className={cn("absolute object-contain animate-float", image.className)}
            style={{ animationDelay: `${index * 300}ms` }}
          />
        ))}
      </div>

      <div className="relative z-20 container mx-auto max-w-2xl px-4 text-center">
